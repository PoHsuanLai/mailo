...
which `crates/mail-app` depends on by path until quire has a public release. Check it out
beside mailo, so that `../quire` from the repository root is quire's:

```sh
git clone https://github.com/PoHsuanLai/quire ../quire
```
