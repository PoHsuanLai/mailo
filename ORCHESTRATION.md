...
518 tests across 45 targets, plus three `#[ignore]`d live tests: a capability probe against
NTU's Dovecot, and two submission tests against a local `aiosmtpd` (`scripts/live-smtpd.py`). `fmt`, `clippy -D warnings` and `scripts/check-boundary.sh` all
clean.
